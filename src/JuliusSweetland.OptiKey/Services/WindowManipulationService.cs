...
namespace JuliusSweetland.OptiKey.Services
{
    public class WindowManipulationService : IWindowManipulationService
    {
        ...
        //Sets the window width to zero and moves window that much left in opposite direction
        public void HideFloatingWindow()
        {
            Log.InfoFormat("HideFloatingWindow called");

            var windowState = getWindowState();
            if (windowState != WindowStates.Floating) return;

            window.Width = 0.0;
            window.Left = 0.0;

            PersistSizeAndPosition();
            App.optikeyFloatingShown = false;
        }

        public void ShowFloatingWindow(bool showInTopHalf, int keyWidthPx, int keyHeightPx)
        {
            Log.InfoFormat("HideFloatingWindow called");
            ...
            //We want optikey to not cover taskbar because sometimes taskbar covers up the spacebar, or any docked bars
            int waHeight = (int)(Screen.GetWorkingArea(Cursor.Position).Height / Graphics.DipScalingFactorY);
            int waWidth = (int)(Screen.GetWorkingArea(Cursor.Position).Width / Graphics.DipScalingFactorX);

            //There are a total of 12 alphabet keys in the current keyboard, gridview of 24 cols, with each key spanning 2 cols
            int minRequiredWidth = (int) Math.Min(waWidth, (keyWidthPx * 24 / 2) / Graphics.DipScalingFactorY);

            //there are a total of 6 gridview rows, and each key spans 1 row in height
            int minRequiredHeight = (int)Math.Min((SystemParameters.PrimaryScreenHeight * 0.40), keyHeightPx * 6 / Graphics.DipScalingFactorY);

            window.Width = minRequiredWidth;
            window.Height = minRequiredHeight;

            //Want to center the keyboard on the screen
            window.Left = Math.Round((double) (waWidth - minRequiredWidth) / 2);
            ...
            {
                //Want it raised above taskbar vertically
                window.Top = waHeight - window.Height;
            }

            PreventWindowActivation(); //does a set window long call.
            BringOptikeyForward();

            App.optikeyFloatingShown = true;
        }

        
        public void BringOptikeyForward()
        {
            //TODO: either by generating a context menu or a quickly closing message box...something that reaches front of the screen
        }
       ...
       /* private void PreventInvalidRestoreState()
        ...
        private void PreventWindowActivation()
        {
            if (Settings.Default.KeySelectionTriggerSource == TriggerSources.MouseButtonDownUps
                || Settings.Default.KeySelectionTriggerSource == TriggerSources.Fixations)
            {
                const int WS_EX_APPWINDOW = 0x40000; //whether it appears in taskbar

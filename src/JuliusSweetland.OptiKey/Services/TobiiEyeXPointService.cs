    ...
    public class TobiiEyeXPointService : IPointService
    {
            ...
            add
            {
                if (pointEvent == null)
                {
                    ...
                    else
                    {
                        ...
                        fixationDataStream.Next += UpdateFixationPosition;
                    }
                }
        ...
        protected void UpdateGazePosition(object s, GazePointEventArgs data)
        {
                    ...
                    && !double.IsNaN(data.Y))
            {
                ...
                p.Offset(CalibrationAdjusterStatic.GetCalibrationAdjustment(p));
                pointEvent(this, new Timestamped<Point>(new Point(p.X, p.Y),
                    new DateTimeOffset(DateTime.UtcNow).ToUniversalTime())); //EyeX does not publish a useable timestamp
            }
        }

        protected void UpdateFixationPosition(object s, FixationEventArgs data)
        {
            if (pointEvent != null
                && !double.IsNaN(data.X)
                && !double.IsNaN(data.Y))
